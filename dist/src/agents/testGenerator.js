 ...
 */
export async function generateTest(prompt, testName, options = {}) {
    ...
    try {
        ...
        throw new TestGenerationError('Failed to generate test', error instanceof Error ? error.message : String(error));
    }
}
/**
 ...
 */
function validateInputs(prompt, testName) {
    if (!prompt?.trim()) {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(testName)) {
        throw new TestGenerationError('Test name contains invalid characters', 'Use only letters, numbers, underscores, and hyphens');
    }
}
/**
...
async function ensureOutputDirectory(outputDir) {
    try {
        ...
        throw new TestGenerationError('Failed to create output directory', error instanceof Error ? error.message : String(error));
    }
}
/**
 * Generates test content using AI with templates
 */
async function generateTestContent(prompt, options) {
    try {
    ...
    `;
        ...
        throw new TestGenerationError('Failed to generate test content', error instanceof Error ? error.message : String(error));
    }
}
/**
 * Validates test syntax and patterns
 */
function validateTestSyntax(content) {
    const warnings = [];
    const checks = [
 ...
 */
async function formatTestCode(content) {
 ...
 */
async function writeTestFile(testDir, testName, content, options) {
    try {
        // Create test file
        ...
        throw new TestGenerationError('Failed to write test file', error instanceof Error ? error.message : String(error));
    }
}
